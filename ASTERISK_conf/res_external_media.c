...
def create_external_media_bridge(channel_id):
    # Create bridge
    bridge_resp = requests.post(
        f"{BASE_URL}/bridges",
        data={"type": "mixing,dtmf_events"},
        auth=HTTPBasicAuth(ARI_USER, ARI_PASSWORD)
    )
    bridge_id = bridge_resp.json()["id"]

    # Create external media channel
    ext_media_resp = requests.post(
        f"{BASE_URL}/channels",
        data={
            "endpoint": "external_media:extapp/1",
            "app": ARI_APP,
            "appArgs": "external",
            "format": "slin16",  # raw signed 16-bit linear PCM
        },
        auth=HTTPBasicAuth(ARI_USER, ARI_PASSWORD)
    )
    ext_channel_id = ext_media_resp.json()["id"]

    # Add both channels to the same bridge
    requests.post(
        f"{BASE_URL}/bridges/{bridge_id}/addChannel",
        data={"channel": f"{channel_id},{ext_channel_id}"},
        auth=HTTPBasicAuth(ARI_USER, ARI_PASSWORD)
    )

    return bridge_id, ext_channel_id
